...
def proveedores (request): 
    if request.method == "POST": 
        ...
        if form.is_valid:
            ...
            proveedor.save()
            return render (request, "appCandyRoom/proveedores.html", {'mensaje' : 'Proveedor cargado '})
    else:
...
def ventas (request): 
    if request.method == "POST": 
        ...
        if form.is_valid:
            ...
            venta.save()
            return render (request, "appCandyRoom/ventas.html", {'mensaje' : 'Venta cargada '})
    else:
        form = VentasForm()
...
def gastos_generales (request): 
    if request.method == "POST": 
        ...
        if form.is_valid:
            ...
            gasto.save()
            return render (request, "appCandyRoom/gastos.html", {'mensaje' : 'Gasto cargado '})
    else:
